...
/**
 * Cindy Eye Care — Contact: Extra Explanation + Hours + Map
 * - Clear “how to reach us” summary
 * - Opening hours (table)
 * - Address + tap-to-call/email
 * - Embedded Google Map + Directions link
 * - Accessibility-first, Tailwind-only
 */

export default function ContactExtrasCindy() {
  const ADDRESS_TEXT = "The Cindy Eye Care, Bwari, Abuja, Nigeria";
  const MAPS_QUERY =
    "https://www.google.com/maps/search/?api=1&query=The+Cindy+Eye+Care%2C+Bwari%2C+Abuja%2C+Nigeria";
  const MAPS_EMBED =
  ...
  return (
    <section className="bg-white text-slate-800">
      ...
      {/* JSON-LD (LocalBusiness) for better SEO — optional */}
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(localBusinessJsonLd(HOURS, ADDRESS_TEXT)) }} />
    </section>
  );
}
...
function localBusinessJsonLd(hours, addressText) {
  // Convert our HOURS array to OpeningHoursSpecification
...
function to24(hhmm) {
  ...
  try {
    ...
    if (mer?.toUpperCase() === "PM" && h !== 12) h += 1 * 12;
